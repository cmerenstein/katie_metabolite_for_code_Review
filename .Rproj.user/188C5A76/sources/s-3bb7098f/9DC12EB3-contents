...
with open('./HMDB/hmdb_metabolites.xml', 'r') as xml:
    tree = ET.parse(xml)
root = tree.getroot()
context = tree.iter(tag='{http://www.hmdb.ca}metabolite')
  
#open a file for writing
csvfile = open('./HMDB/hmdb_metabolites.csv', 'wb')
...
#Read in input data
data = pd.read_excel('./Results/literature_metabolites.xlsx', engine = 'openpyxl')
...
#create object
with open('./Results/lit_metabolites_key.csv', 'wt') as f:
    writer = csv.writer(f)
